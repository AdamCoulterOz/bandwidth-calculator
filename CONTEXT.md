...
- Field, selector, option, and disclosure-summary hover/focus fills use the Keel sunken-surface token. This preserves neutral resting surfaces while keeping interaction visibly distinct from the shared group background in both system themes.
...
- The full-width header is a compact 64-pixel frosted Keel navigation surface. Its subtitle remains visible above 1100px and is hidden at narrower widths, while the transparent monitor brand mark remains visible at every viewport width and scales down with the header. Reset is a blue-tinted filled pill without a resting outline, and the configuration panel uses a shallow top inset so its leading diagram sits close below the header. Configuration and results remain side by side through compact-desktop widths and stack below 960px. Within the stacked workspace, widths from 760px through 960px use an internal two-column configuration projection: diagram left, primary fields right, and the two disclosure panels paired below; narrower widths return to the linear single-column flow.
